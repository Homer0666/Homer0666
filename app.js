(function () {
  ...
  function renderLive(now) {
    ...
    $('homeHenchSub').textContent = s >= 200 ? 'TECHO ALCANZADO' : s <= 100 ? 'PISO ALCANZADO' : 'RANGO 100 · 200';
  }
  ...
  function radarCycle(now) {
    var k = Math.floor(now / 30000);
    if (k === radarK) return;
    radarK = k;
    var r = mulberry32(hashSeed(k * 401 + 9));
    var ang = r() * Math.PI * 2;
    var rad = 0.12 + r() * 0.7;
    var x = 50 + rad * 40 * Math.cos(ang);
    var y = 50 + rad * 40 * Math.sin(ang);
    if (radarK <= 1) {
      blipEl.style.transition = 'none';
      blipEl.style.left = x + '%';
      blipEl.style.top = y + '%';
  ...
  function renderCam(now) {
    ...
    if (audioOn && Date.now() < camNextAt) {
      var left = Math.ceil((camNextAt - Date.now()) / 1000);
      $('soundTimer').textContent = '0:' + fmt(Math.max(0, left));
    } else if (audioOn) {
      $('soundTimer').textContent = 'SONANDO...';
    }
  }
  function onCamsOpen() {
    var v = $('camVideo');
