(function () {
  ...
  var ATK_CAUSES = [
    ...
    'PLANES SALIENDO MAL',
    'LA RADIO FNAF SE DESINCORPORÓ',
    'SE TERMINARON LOS DÓNUTS DEL TÚNEL',
    'LA FURGONETA BLANCA PINCHÓ EN PLENA MISIÓN',
    'UN SECUAZ SE BURLÓ DE SU BIGOTE',
    'EL OJO LO MIRÓ DOS VECES SEGUIDAS',
    'LAS MESAS EXTRAÍBLES ESTÁN OCUPADAS',
    'REVELARON EL PLAN SECRETO DE FIESTA'
  ];
  var moodCacheB = -1;
  var angerVal = 40;
  var atkVal = 5;
  ...
  function moodWalk(b) {
    ...
    for (var i = moodCacheB + 1; i <= b; i++) {
      ...
      /* medidor de ataques de ira: +1 normal, a veces +2..+5, raro +25, a veces baja */
      var roll = rag();
      var d;
      if (roll < 0.03) d = 25;
      else if (roll < 0.18) d = 2 + Math.floor(rag() * 4);
      else if (roll < 0.35) d = -(1 + Math.floor(rag() * 3));
      else d = 1;
      atkVal += d;
  ...
  function overloadFx() {
    ...
    try {
      ...
      if (AC) {
        ...
        g.gain.value = 0.12;
        o.start();
        setTimeout(function () { try { o.stop(); ctx.close(); } catch (e) {} }, 900);
      }
    } catch (e) {}
  }
  function overloadClear() {
    ...
    overloadFxOn = false;
    var b = document.body;
    if (b) b.classList.remove('overload-shake');
    if (navigator.vibrate) {
  ...
  function renderAnger(now) {
    ...
    moodWalk(b);
    var a = angerVal;
    var overload = atkActive > 0 && atkCause === 'MEQUETREFES INSULTÁNDOME';
    ...
    /* fuerza estimada: 0% → 250 kg (mínimo), 100% → 425 kg · sobrecarga → 666 kg */
    var force = overload ? 666 : Math.round(250 + (a / 100) * 175);
    var fill = $('angerFill');
    if (fill) {
      fill.style.width = a + '%';
